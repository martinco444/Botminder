...
async def start(update: Update, context: CallbackContext):
    ...
    logger.info("Comando /start user_id=%s username=%s", user.id, user.username)
    await update.message.reply_text("¡Hola! Soy tu bot de recordatorios. Usa /recordatorio para guardar uno.")

def pedir_mensaje(update: Update, context: CallbackContext) -> int:
    update.message.reply_text("¿Qué necesitas recordar?")
    logger.debug("Datos recibidos: %s", update.message.text)
    return PEDIR_FECHA
